...
if(!function_exists('houzez_search_radius_api')) {
    function houzez_search_radius_api($meta_query) {
    ...
    add_filter('houzez_meta_search_filter', 'houzez_search_radius_api');
}


### Added By Inan Celis
// Alternative: Add to the half map listings function specifically
if(!function_exists('houzez_halfmap_auto_price_ordering')) {
    function houzez_halfmap_auto_price_ordering() {
        // Check if min-price or max-price parameters exist in URL
        $has_min_price = isset($_GET['min-price']) && !empty($_GET['min-price']) && $_GET['min-price'] != 'any';
        $has_max_price = isset($_GET['max-price']) && !empty($_GET['max-price']) && $_GET['max-price'] != 'any';
        
        // Return ordering parameters if price filters are active and no manual sort is set
        if (($has_min_price || $has_max_price) && (!isset($_GET['sortby']) || empty($_GET['sortby']))) {
            return array(
...
if( !function_exists('houzez20_half_map_listings') ) {
    function houzez20_half_map_listings() {
        ...
        $auto_price_order = houzez_halfmap_auto_price_ordering();

        if ($auto_price_order !== false) {
            // Apply automatic price ordering
            $search_qry['orderby'] = $auto_price_order['orderby'];
            $search_qry['meta_key'] = $auto_price_order['meta_key'];
            $search_qry['order'] = $auto_price_order['order'];
        } else {
            // Set default sort to newest first if no sort is specified
            if (empty($sort_by)) {
                $sort_by = 'd_date';
            }
...
if(!function_exists('houzez_search_states')) {
	function houzez_search_states($query_arg) {
	...
	add_filter('houzez_taxonomy_search_filter', 'houzez_search_states');
}

if(!function_exists('houzez_search_state')) {
    function houzez_search_state($query_arg) {

        if (isset($_GET['state']) && !empty($_GET['state']) && !empty($_GET['state'][0]) && $_GET['state'] != 'all' && $_GET['state'] != '-1' && $_GET['state'][0] != '-1' ) {
            $query_arg[] = array(
                'taxonomy' => 'property_state',
                'field' => 'slug',
                'terms' => $_GET['state']
...
if(!function_exists('houzez_search_location')) {
	function houzez_search_location($query_arg) {
        ...
        // Changed by Inan Celis 08/27/2025
        if (isset($_GET['location']) && !empty($_GET['location']) && !empty($_GET['location'][0]) && $_GET['location'] != 'all' && $_GET['location'] != '-1' && $_GET['location'][0] != '-1') {
            
            // Create an OR relationship between multiple taxonomies
            ...
            $query_arg[] = $location_query;
        }

        return $query_arg;
	}

	add_filter('houzez_taxonomy_search_filter', 'houzez_search_location');
}

if(!function_exists('houzez_search_city')) {
    function houzez_search_city($query_arg) {

        if (isset($_GET['city']) && !empty($_GET['city']) && !empty($_GET['city'][0]) && $_GET['city'] != 'all' && $_GET['city'] != '-1' && $_GET['city'][0] != '-1') {
            $query_arg[] = array(
                'taxonomy' => 'property_city',
                'field' => 'slug',
                'terms' => $_GET['city']
...
if(!function_exists('houzez_search_areas')) {
	function houzez_search_areas($query_arg) {
	...
	add_filter('houzez_taxonomy_search_filter', 'houzez_search_areas');
}

if(!function_exists('houzez_search_area')) {
    function houzez_search_area($query_arg) {

        if (isset($_GET['area']) && !empty($_GET['area']) && !empty($_GET['area'][0]) && $_GET['area'] != 'all' && $_GET['area'] != '-1' && $_GET['area'][0] != '-1' ) {
            $query_arg[] = array(
                'taxonomy' => 'property_area',
                'field' => 'slug',
                'terms' => $_GET['area']
...
if(!function_exists('houzez_search_by_id')) {
    function houzez_search_by_id($meta_query) {
        if (isset($_GET['ids']) && !empty($_GET['ids'])) {
            $ids = is_array($_GET['ids']) ? $_GET['ids'] : explode(',', $_GET['ids']);
            $ids = array_map('intval', $ids); // Ensure all values are integers
            $meta_query['post__in'] = $ids;
        }
        return $meta_query;
    }
    add_filter('houzez20_search_filters', 'houzez_search_by_id');
}

if(!function_exists('houzez_search_by_user')) {
    function houzez_search_by_user($meta_query) {
        if (isset($_GET['user_id']) && !empty($_GET['user_id'])) {
            $meta_query['author'] = intval($_GET['user_id']);
        }
        return $meta_query;
    }
    add_filter('houzez20_search_filters', 'houzez_search_by_user');
}

if(!function_exists('houzez_search_by_post_status')) {
    function houzez_search_by_post_status($meta_query) {
        if (isset($_GET['post_status'])) {
            if (is_array($_GET['post_status'])) {
                $meta_query['post_status'] = array_map('sanitize_text_field', $_GET['post_status']);
            } else {
                $meta_query['post_status'] = sanitize_text_field($_GET['post_status']);
            }
        }
        return $meta_query;
    }
    add_filter('houzez20_search_filters', 'houzez_search_by_post_status');
}

if(!function_exists('houzez_search_by_agents')) {
    function houzez_search_by_agents($meta_query) {
        // Filter by agent
    ...
    add_filter('houzez_meta_search_filter', 'houzez_search_by_agents');
}


if(!function_exists('houzez_houzez_search_by_agency')) {
    function houzez_houzez_search_by_agency($meta_query) {
       
        if (isset($_GET['agency_id']) && !empty($_GET['agency_id'])) {
            $agency_ids = is_array($_GET['agency_id']) ? $_GET['agency_id'] : explode(',', $_GET['agency_id']);
            $agency_ids = array_map('intval', $agency_ids);
    ...
    add_filter('houzez_meta_search_filter', 'houzez_houzez_search_by_agency');
}

if(!function_exists('houzez_search_by_featured')) {
    function houzez_search_by_featured($meta_query) {
    ...
    add_filter('houzez_meta_search_filter', 'houzez_search_by_featured');
}

if(!function_exists('houzez_search_by_user_role')) {
    function houzez_search_by_user_role($meta_query) {
        if (isset($_GET['user_role']) && !empty($_GET['user_role'])) {
            ...
            if (!empty($users)) {
                ...
                if (!empty($user_ids)) {
                    $meta_query['author__in'] = $user_ids;
                }
            } else {
                // If no users found with this role, return no results
                $meta_query['author__in'] = array(0); // This will return no results
            }
        }
        return $meta_query;
    }
    add_filter('houzez20_search_filters', 'houzez_search_by_user_role');
}
