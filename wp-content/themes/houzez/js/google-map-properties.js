  ...
  if (typeof houzez_map_properties !== "undefined") {
    if ($("#houzez-properties-map").length > 0) {
      ...
      var InfoboxTrigger = function () {
        ...
        $("#half-map-listing-area .item-listing-wrap").on(
          "mouseleave",
          function () {
            hideInfoWindows();
            if (map_cluster_enable != 0) {
      ...
      var houzez_half_map_listings = function (current_page) {
        ...
        $.ajax({
          ...
          success: function (data) {
            //alert(JSON.stringify(data.query)); return;
            ...
            if (data.getProperties === true) {
              ...
              houzezMap.setCenter(
                new google.maps.LatLng(default_lat, default_long)
              );
              houzezMap.setZoom(7);
              ...
              ajax_container
                .empty()
                .html(
                  '<div class="search-no-results-found">' + not_found + "</div>"
                );
              total_results.empty().html(data.total_results);
            }
            return false;
